...
pub fn wallet_button() -> Html {
    ...
    html! {
        <>
        <md-checkbox {onclick} disabled={!eth.walletconnect_available()} />
        <label>{"Wallet connect chain: "}{eth.chain_id()}</label>
        //<button onclick={onclick_ethereum}>{label}</button>
        <md-text-button onclick={onclick_ethereum}>{label}</md-text-button>
        </>
    }
}
